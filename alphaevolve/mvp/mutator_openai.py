...
def _load_api_key() -> str | None:
    ...
    for parents_up in range(1, 4):
        dotenv_path = Path(__file__).resolve().parents[parents_up] / ".env"
...
def _candidate_fallback_models(model_name: str) -> list[str]:
    ...
    if model_name == "gpt-5.4":
        fallbacks.extend(["gpt-5.2", "gpt-5"])
    elif model_name == "gpt-5.4-mini":
        fallbacks.extend(["gpt-5.2-mini", "gpt-5-mini", "gpt-4.1-mini"])
    elif model_name == "gpt-5.2-mini":
        fallbacks.extend(["gpt-5-mini", "gpt-4.1-mini"])
    elif model_name == "gpt-5.2":
        fallbacks.extend(["gpt-5", "gpt-5-mini"])

    # Generic fallback: strip minor version.
    for seg in [".4", ".2"]:
        if seg in model_name:
            fallbacks.append(model_name.replace(seg, ""))
            break
        ...
        for model_try in [selected_model] + _candidate_fallback_models(selected_model):
            ...
            for attempt in range(self.max_retries + 1):
                ...
                try:
                    ...
                    # Use xhigh reasoning for gpt-5.4, medium for others
                    is_heavy = "5.4" in model_try
                    reasoning_effort = "xhigh" if is_heavy else "high"

...
def _make_kernel(config: helion.Config):
    ...
    def concat_chunk_kernel(A_c: torch.Tensor, x_cat_c: torch.Tensor, chunk_size: int) -> torch.Tensor:
        batch, nchunks, nheads, _, dcat = x_cat_c.shape
        chunk_size = hl.specialize(chunk_size)
        hl.specialize(dcat)
        acc_dtype = torch.float32
        dtype = x_cat_c.dtype
        out = torch.empty_like(x_cat_c)
        block_d = hl.register_block_size(dcat)
        for tile_b, tile_c, tile_h, tile_d in hl.tile(
            [batch, nchunks, nheads, dcat], block_size=[1, 1, 1, block_d]
        ):
            i_b = tile_b.id
            i_c = tile_c.id
            i_h = tile_h.id
            out[i_b, i_c, i_h, :, tile_d] = hl.dot(
                ...
                x_cat_c[i_b, i_c, i_h, :, tile_d].to(acc_dtype),
                out_dtype=acc_dtype,
            ).to(dtype)
        return out

    return concat_chunk_kernel
...
def custom_kernel(data: input_t) -> output_t:
    ...
    out_cat = kernel(A_c, torch.cat([k_c, v_c], dim=-1), CHUNK_SIZE)
    return _unchunk_x(out_cat[..., :K]), _unchunk_x(out_cat[..., K:])

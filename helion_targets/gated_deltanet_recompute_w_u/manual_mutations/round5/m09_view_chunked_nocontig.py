        ...
        for tile_b, tile_c, tile_h in hl.tile([batch, nchunks, nheads], block_size=[1, 1, 1]):
            ...
            for tile_k in hl.tile(dk, block_size=block_k):
                out_w[i_b, i_c, i_h, :, tile_k] = hl.dot(
                    b_A, k_c[i_b, i_c, i_h, :, tile_k].to(acc_dtype), out_dtype=acc_dtype
                ).to(dtype)
            for tile_v in hl.tile(dv, block_size=block_v):
                out_u[i_b, i_c, i_h, :, tile_v] = hl.dot(
                    b_A, v_c[i_b, i_c, i_h, :, tile_v].to(acc_dtype), out_dtype=acc_dtype
                ).to(dtype)
        return out_w, out_u
...
def _chunk_a(A: torch.Tensor, chunk_size: int) -> torch.Tensor:
    ...
    return A.reshape(B, nchunks, chunk_size, H, chunk_size).permute(0, 1, 3, 2, 4)


def _chunk_x(x: torch.Tensor, chunk_size: int) -> torch.Tensor:
    B, T, H, D = x.shape
    nchunks = T // chunk_size
    return x.reshape(B, nchunks, chunk_size, H, D).permute(0, 1, 3, 2, 4)
...
def custom_kernel(data: input_t) -> output_t:
    ...
    return tuple(_unchunk_x(x) for x in kernel(_chunk_a(A, CHUNK_SIZE), k_c, v_c, CHUNK_SIZE))

...
def _make_kernel(config: helion.Config):
    ...
    def chunk_matmul_kernel(
        ...
        for tile_b, tile_c, tile_h, tile_d in hl.tile(
            [batch, nchunks, nheads, dout], block_size=[1, 1, 1, block_d]
        ):
            i_b = tile_b.id
            i_c = tile_c.id
            i_h = tile_h.id
            ...
            b_x = x_c[i_b, i_c, i_h, :, tile_d].to(acc_dtype)
            b_out = hl.dot(b_A, b_x, out_dtype=acc_dtype)
            out[i_b, i_c, i_h, :, tile_d] = b_out.to(dtype)
...
SHAPE_CONFIGS: dict[tuple, tuple[helion.Config, helion.Config]] = {
    (1, 64, 2, 64, 64): (
        helion.Config(block_sizes=[64], flatten_loops=True, num_warps=4, num_stages=1),
        helion.Config(block_sizes=[64], flatten_loops=True, num_warps=4, num_stages=1),
    ),
    (2, 128, 4, 64, 64): (
        helion.Config(block_sizes=[64], flatten_loops=True, num_warps=4, num_stages=1),
    ...
    (1, 256, 4, 64, 128): (
        helion.Config(block_sizes=[64], flatten_loops=True, num_warps=4, num_stages=2),
        helion.Config(block_sizes=[64], flatten_loops=True, num_warps=8, num_stages=2),
    ),
    (1, 64, 1, 64, 64): (
        helion.Config(block_sizes=[64], flatten_loops=True, num_warps=4, num_stages=2),
        helion.Config(block_sizes=[64], flatten_loops=True, num_warps=4, num_stages=2),
    ...
    (2, 1024, 3, 64, 64): (
        helion.Config(block_sizes=[64], flatten_loops=True, num_warps=8, num_stages=3),
        helion.Config(block_sizes=[64], flatten_loops=True, num_warps=8, num_stages=3),
    ),
}

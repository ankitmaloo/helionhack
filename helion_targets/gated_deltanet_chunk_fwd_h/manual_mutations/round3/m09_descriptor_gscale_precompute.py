    ...
    def chunk_fwd_h_kernel(
        ...
        for tile_b, tile_h, tile_v in hl.tile([batch, nheads, dstate], block_size=[1, 1, block_v]):
            ...
            for t_i in hl.tile(seqlen, block_size=chunk_size):
                ...
                gate_scale = torch.exp(g_last - g_chunk)
                b_v_raw = u[i_b, t_i, i_h, tile_v].to(acc_dtype) - hl.dot(w_chunk, b_h_store, out_dtype=acc_dtype)
                v_new_out[i_b, t_i, i_h, tile_v] = b_v_raw.to(dtype)
                b_h *= decay
                p_k = k[i_b, t_i, i_h, :]
                b_h = hl.dot(p_k.T, (b_v_raw * gate_scale[:, None]).to(dtype), acc=b_h)

...
def _make_kernel(config: helion.Config):
    ...
    def chunk_fwd_h_kernel(
        ...
        for tile_b, tile_h, tile_v in hl.tile([batch, nheads, dstate], block_size=[1, 1, block_v]):
            i_b = tile_b.id
            i_h = tile_h.id
            # Start b_h in acc_dtype but try to keep more things in dtype
            ...
            for t_i in hl.tile(seqlen, block_size=chunk_size):
                ...
                b_w = w[i_b, t_i, i_h, :]
                b_v = hl.dot(b_w, b_h_store, out_dtype=dtype)
                p_v = u[i_b, t_i, i_h, tile_v]
                b_v = p_v - b_v
                v_new_out[i_b, t_i, i_h, tile_v] = b_v
                
                t_i_last = t_i.begin + chunk_size - 1
                b_g_last = g[i_b, t_i_last, i_h]
                b_g = g[i_b, t_i, i_h]
                
                # Math in dtype to reduce casting operations
                g_diff = b_g_last - b_g
                scale_v = torch.exp(g_diff.to(acc_dtype))[:, None].to(dtype)
                scale_h = torch.exp(b_g_last.to(acc_dtype))

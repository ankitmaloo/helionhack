    ...
    def chunk_fwd_h_kernel(
        ...
        for tile_b, tile_h, tile_v in hl.tile([batch, nheads, dstate], block_size=[1, 1, block_v]):
            ...
            for t_i in hl.tile(seqlen, block_size=chunk_size):
                ...
                # Winner m03 math from round 4
                g_diff = b_g_last - b_g
                scale_v = torch.exp(g_diff)[:, None]
                scale_h = torch.exp(b_g_last)
...
SHAPE_CONFIGS: dict[tuple, helion.Config] = {
    ...
    # Push warps to 16 for the largest shape
    (2, 512, 3, 64, 64): helion.Config(block_sizes=[64], indexing="tensor_descriptor", num_warps=8, num_stages=2),
    (2, 1024, 3, 64, 64): helion.Config(block_sizes=[64], indexing="tensor_descriptor", num_warps=16, num_stages=2),
}
# EVOLVE-BLOCK-END
